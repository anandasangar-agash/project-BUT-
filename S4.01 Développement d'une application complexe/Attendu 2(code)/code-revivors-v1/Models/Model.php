...
class Model
{
    ...
    {
        $req = $this->bd->prepare('SELECT * FROM typevoie;');
        $req->execute();
        return $req->fetchAll(PDO::FETCH_ASSOC);
    }

    public function getIdTypeVoie($libelle) {
        $req = $this->bd->prepare('SELECT id_type_voie FROM TypeVoie WHERE libelle = :libelle');
    ...
    {
        $req = $this->bd->prepare('SELECT id_composante FROM COMPOSANTE WHERE nom_composante = :composante');
    ...
    {
        $req = $this->bd->prepare('SELECT id_personne FROM PERSONNE WHERE email=:email');
        $req->bindValue(':email', $email);
        $req->execute();
        return $req->fetch(PDO::FETCH_ASSOC);
    }

    public function getIdComposanteByIdClient($id_client)
    {
        $req = $this->bd->prepare('SELECT id_composante FROM COMPOSANTE WHERE id_client = :id_client');
     ...
     * @return bool
     */
    public function assignerInterlocuteurComposanteByIdComposante($id_composante, $id_personne)
    {
        $req = $this->bd->prepare("INSERT INTO dirige (id_personne, id_composante) VALUES (:id_composante, :id_personne)");
     ...
     * @return bool
     */
    public function assignerInterlocuteurComposanteByIdClient($id_personne, $composante)
    {
        $req = $this->bd->prepare("INSERT INTO dirige (id_personne, id_composante) VALUES (:id_composante, :id_personne)");
        $req->bindValue(':id_composante', $composante, PDO::PARAM_INT);
    ...
    {
        $req = $this->bd->prepare("CALL ajouter_client(:client, :tel);");
     ...
     * @return bool
     */
    public function addComposante($libelleVoie, $id_type_voie, $cp, $numVoie, $nomVoie, $nom_client, $nom_compo)
    {
        $req = $this->bd->prepare("INSERT INTO ADRESSE(numero, nom_voie, id_type_voie, id_localite) SELECT :num, :nomVoie, :id_type_voie, (SELECT id_localite FROM localite WHERE cp = :cp)");
        $req->bindValue(':num', $numVoie, PDO::PARAM_STR);
        $req->bindValue(':nomVoie', $nomVoie, PDO::PARAM_STR);
        $req->bindValue(':id_type_voie', $id_type_voie, PDO::PARAM_STR);
     ...
     * @return bool
     */
    public function addMission($type, $nom, $date, $nom_client, $id_composante)
    {
        $req = $this->bd->prepare("INSERT INTO MISSION (type_bdl, nom_mission, date_debut, id_composante) VALUES (:type, :nom, :date, :id_composante)");
        $req->bindValue(':nom', $nom);
    ...
    public function checkMailPassword($mail, $password)
    {
        ...
        if ($realPassword) {
            if (password_verify($password,$realPassword['mdp'])) {
                if (isset($_SESSION)) {
                    session_destroy();
                }
                ...
                $_SESSION['id'] = $realPassword['id_personne'];
                $_SESSION['nom'] = $realPassword['nom'];
                $_SESSION['prenom'] = $realPassword['prenom'];
                $_SESSION['email'] = $realPassword['email'];
                return true;
            }
        }
        return false;
